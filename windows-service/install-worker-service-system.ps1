...
# Configured to run as SYSTEM (LocalSystem) account
...
Write-Host "Installing Nexrender Worker as Windows Service (SYSTEM account)..." -ForegroundColor Green
...
Write-Host "Account: LocalSystem (SYSTEM)" -ForegroundColor Yellow
Write-Host ""
Write-Host "WARNING: Running as SYSTEM account may cause issues with:" -ForegroundColor Yellow
Write-Host "  - User profile paths (fonts, temp files)" -ForegroundColor Yellow
Write-Host "  - After Effects user preferences" -ForegroundColor Yellow
Write-Host "  - Network drive access" -ForegroundColor Yellow
Write-Host ""
...
& $NSSMPath set $ServiceName Description "Nexrender Worker - Processes After Effects rendering jobs (Running as SYSTEM)"
...
# Configure service to run as SYSTEM (LocalSystem)
# Use "LocalSystem" for SYSTEM account
...
Write-Host "`nService installed successfully as SYSTEM account!" -ForegroundColor Green
...
Write-Host "`nTo change back to user account, run:" -ForegroundColor Yellow
Write-Host "  & '$NSSMPath' set $ServiceName ObjectName `"$([System.Security.Principal.WindowsIdentity]::GetCurrent().Name)`"" -ForegroundColor Cyan
